...
| --------------------------------- | ------------------------------------------------------- | ------------------------------------------------ |
| `com.ichi2.anki.hmos`         | HMOS bundle name (reverse-DNS, ends in `.hmos`)         | `com.beemdevelopment.aegis.hmos`                 |
| `AnkiDroid`              | Vendor / publisher name (usually app name)              | `Aegis`                                          |
| `1`        | Integer version code                                    | `1`                                              |
| `1.0.0`        | Semver version string                                   | `1.0.0`                                          |
| `AnkiDroid`           | Display name shown on the launcher                      | `Aegis`                                          |
| `ankidroid`      | Lowercase, no-space identifier for `oh-package.json5`   | `aegis`                                          |
| `AnkiDroid flashcards for HarmonyOS` | One-line project description                            | `Aegis 2FA authenticator for HarmonyOS`          |
| `AnkiDroid main module`         | Description for `entry/src/main/module.json5` (module)  | `Aegis main module`                              |
| `AnkiDroid main entry ability`        | Description for `entry/src/main/module.json5` (ability) | `Aegis main entry ability`                       |
...
find "${DEST}" -type f \( -name '*.json5' -o -name '*.json' -o -name '*.ts' -o -name '*.ets' -o -name '*.md' \) \
  -exec sed -i \
    -e 's|com.ichi2.anki.hmos|com.beemdevelopment.aegis.hmos|g' \
    -e 's|AnkiDroid|Aegis|g' \
    -e 's|1|1|g' \
    -e 's|1.0.0|1.0.0|g' \
    -e 's|AnkiDroid|Aegis|g' \
    -e 's|ankidroid|aegis|g' \
    -e 's|AnkiDroid flashcards for HarmonyOS|Aegis 2FA authenticator for HarmonyOS|g' \
    -e 's|AnkiDroid main module|Aegis main module|g' \
    -e 's|AnkiDroid main entry ability|Aegis main entry ability|g' \
    {} +
```
...
$map = @{
  'com.ichi2.anki.hmos'         = 'com.beemdevelopment.aegis.hmos'
  'AnkiDroid'              = 'Aegis'
  '1'        = '1'
  '1.0.0'        = '1.0.0'
  'AnkiDroid'           = 'Aegis'
  'ankidroid'      = 'aegis'
  'AnkiDroid flashcards for HarmonyOS' = 'Aegis 2FA authenticator for HarmonyOS'
  'AnkiDroid main module'         = 'Aegis main module'
  'AnkiDroid main entry ability'        = 'Aegis main entry ability'
}
